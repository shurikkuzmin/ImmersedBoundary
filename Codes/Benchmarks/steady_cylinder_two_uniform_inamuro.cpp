...
void update_inamuro()
{
    ...
    for(int iY=0;iY<NY;iY++)
    {
    	...
    	double rho_wall  = 1.0/(1.0-vel_center)*(f[offset+0]+f[offset+2]+f[offset+4]+
    	                   2.0*(f[offset+3]+f[offset+6]+f[offset+7]));
        double rho_prime = 6.0/(1.0+3.0*vel_center+3.0*vel_center*vel_center)*
                           (rho_wall*vel_center+(f[offset+3]+f[offset+6]+f[offset+7]));
    	
    	f[offset+1]=weights[1]*rho_prime*(1.0+3.0*vel_center+3.0*vel_center*vel_center);
...
void calculate_mass()
{
	...
	for(int counter=0;counter<NUM;counter++)
		sum_rho += rho[counter];
	std::cout << "Overall mass: " << sum_rho << std::endl;
}
